...
// ✅ Client helper for Next.js App Router
...
export const supabaseServer = (() => {
  ...
  if (!supabaseServiceKey) {
    console.warn('⚠️ Missing SUPABASE_SERVICE_ROLE_KEY — server features may fail.');
    return null;
  }

  console.log('🔐 Supabase service role key prefix:', supabaseServiceKey.slice(0, 6));
...
export async function getUserStats(userId: string) {
  ...
  const [{ count: datasetsCount }, { count: verifiedCount }, { count: reviewsCount }, { data: userData }] =
    await Promise.all([
