 ...
 * -------------------------------------------------------------
 * Authenticates via Supabase + Authorization header,
 * Inserts a human review for a dataset,
 * Updates dataset verification score,
 * Awards reviewer + uploader points.
 * =============================================================
 */
export async function POST(req: NextRequest) {
  try {
    // ✅ Brutal Fix: Await cookies (dynamic API route)
    const cookieStore = await cookies();
    const supabase = createRouteHandlerClient({ cookies: () => cookieStore } as any);

    // ✅ Support Authorization header for API calls
    ...
    // 11️⃣ AWARD UPLOADER BONUS (+200 if verified)
    if (isVerified) {
      const { error: uploaderRewardError } = await supabase.rpc(
        "award_points",
        {
          p_user_id: dataset.uploader_id,
          p_points: 200,
