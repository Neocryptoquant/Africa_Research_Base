...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: dataset, error: datasetError } = await supabaseServer!
      .from("datasets")
      .select(
        "id, uploader_id, ai_confidence_score, total_reviews, human_verification_score, is_verified"
      )
      .eq("id", datasetId)
      .single();
    ...
    return NextResponse.json({
      ...
      message: `Review submitted! You earned ${reviewerPoints} points!${
        uploaderBonusAwarded
          ? ` The uploader received ${REVIEW_POINTS.VERIFICATION_BONUS} points verification bonus!`
          : ""
      }`,
    });
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reviewedDatasetIds = new Set(
      userReviews?.map((r) => r.dataset_id) || []
    );

    // Filter out already reviewed datasets
    const pendingDatasets =
      datasets?.filter((dataset) => !reviewedDatasetIds.has(dataset.id)) || [];
