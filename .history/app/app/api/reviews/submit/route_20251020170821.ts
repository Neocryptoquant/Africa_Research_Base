  ...
  try {
    ...
    if (isVerified && wasNotVerifiedBefore) {
      ...
      if (uploaderProfile?.wallet_address) {
        try {
          const uploaderPublicKey = new PublicKey(
            uploaderProfile.wallet_address
          );
          ...
          console.log(
            `✅ Awarded ${bonusReward} $ARB verification bonus to uploader`
          );
        } catch (tokenError) {
    ...
    if (reviewerProfile?.wallet_address) {
      try {
        const reviewerPublicKey = new PublicKey(reviewerProfile.wallet_address);
        reviewerBalance =
          await arbTokenService.getTokenBalance(reviewerPublicKey);
      } catch (error) {
    ...
    return NextResponse.json({
      ...
      uploaderBonus:
        isVerified && wasNotVerifiedBefore
          ? {
